...
function _buildright(projψ::ProjMPS, site::Int)
    ...
    ten = contract(projψ.objects[begin][site], ten, (2, 3), (2, ndims(ten)),
        !isconj(projψ.objects[begin]), false)
    return ten
end
...
function inner(projψ::ProjMPS)
    ...
    return projψ.λ * contract(left, right, Base.OneTo(ndims(left)), Base.OneTo(ndims(right)))[]
end
...
function gradient(projψ::ProjMPS, A::AbstractArray, dir::Bool=false)
    ...
    if projψ.sym
        ...
        for i in Base.range(firstsite, lastsite)
            left = contract(left, projψ.objects[begin][i], 1, 1, false, !isconj(projψ.objects[begin]))
            left = permutedim(left, ndims(left), 1)
        end
    end
    ...
    grad = contract(left, right, Base.OneTo(length(projψ.objects)-1), Base.OneTo(length(projψ.objects)-1))
    grad .*= projψ.λ
    return grad
end
