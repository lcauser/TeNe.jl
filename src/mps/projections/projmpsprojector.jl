...
function inner(projψ::ProjMPSProjectors, A::AbstractArray, dir::Bool=false)
    ...
    # Contract with first
    if projψ.sym
        prod *= contract(A, gradientconj(projψ.projections[1], A, dir), Base.OneTo(ndims(A)),
            Base.OneTo(ndims(A)), !isconj(projψ.projections[1]))[]
    else
...
function product(projψ::ProjMPSProjectors, A::AbstractArray, dir::Bool=false; tocache::Bool=true)
